// REHEARSAL ONLY. Generated from src/; do not treat as the submission artifact.
// Modules retain their original crate-root names so existing crate:: imports remain valid.
...
///
/// Every variant here must correspond to a documented rejection path â€”
...
pub enum ProtocolError {
    ...
    /// â€” connection.rs should keep buffering and retry once more bytes
    /// arrive.
    Incomplete,
    /// The "Remaining Length" field (MQTT 3.1.1 Â§2.2.3) was malformed
    /// (e.g. more than 4 continuation bytes).
    InvalidRemainingLength,
    /// The fixed header's packet type nibble did not match any known
    /// MQTT 3.1.1 control packet type (Â§2.2.1).
    UnknownPacketType(u8),
    /// A packet type or feature not in this broker's core subset
    /// (e.g. QoS 2 PUBLISH) â€” see PLAN.md Â§3/Â§4 for scope.
...
// ===== BEGIN src/logging.rs =====
mod logging {
//! A minimal hand-rolled leveled logger â€” see STDLIB.md (`log`/`tracing`
...
// ===== BEGIN src/protocol.rs =====
mod protocol {
//! MQTT 3.1.1 wire protocol â€” packet types and (de)serialization.
//!
//! Owner: Role B. The enum/struct shapes below are the stable contract
//! Role A's connection-handling code (connection.rs) is built against â€”
...
//! Core scope (PLAN.md Â§3): CONNECT/CONNACK, SUBSCRIBE/SUBACK,
//! UNSUBSCRIBE/UNSUBACK, PUBLISH (QoS 0 only), PINGREQ/PINGRESP,
//! DISCONNECT. Extras (Â§4): wildcards, QoS 1, retained/will messages.
...
pub struct ConnectPacket {
    ...
    // username/password/will fields: out of core scope â€” add if pursuing
    // the "last-will messages" extra (PLAN.md Â§4).
}

/// Connect return codes, MQTT 3.1.1 Â§3.2.2.3.
...
/// MQTT 3.1.1 Â§3.11 UNSUBACK.
#[derive(Debug, Clone)]
pub struct UnsubAckPacket {
    pub packet_id: u16,
}

/// MQTT 3.1.1 Â§3.3 PUBLISH. Core scope: QoS 0 only (`qos` is always 0,
/// `packet_id` is always `None` â€” a QoS 0 PUBLISH has no packet
/// identifier per spec). Extra scope (PLAN.md Â§4 item 2): `qos == 1`
...
/// MQTT 3.1.1 Â§3.4 PUBACK â€” extra scope (PLAN.md Â§4 item 2). Sent to
...
// --- Packet type nibble values, MQTT 3.1.1 Â§2.2.1 Table 2.1 ---
...
/// packet â€” the caller (connection.rs) keeps buffering and retries, this
...
/// adversarial byte sequences â€” AI_GUARDRAILS.md rule 3.
pub fn decode(buf: &[u8]) -> Result<(MqttPacket, usize), ProtocolError> {
    // Fixed header, Â§2.2: byte 0 = packet type (high nibble) + flags (low
    ...
    let packet = match packet_type {
        ...
        PT_PINGREQ => {
            if !body.is_empty() {
                return Err(ProtocolError::MalformedPayload(
                    "PINGREQ must have zero remaining length (Â§3.13.1)",
                ));
            }
            MqttPacket::PingReq
        }
        PT_DISCONNECT => {
            if !body.is_empty() {
                return Err(ProtocolError::MalformedPayload(
                    "DISCONNECT must have zero remaining length (Â§3.14.1)",
                ));
            }
...
pub fn encode(packet: &MqttPacket) -> Vec<u8> {
    match packet {
        ...
        // don't panic â€” just produce the correct bytes.
...
/// Decode the MQTT "Remaining Length" variable-length field (Â§2.2.3)
...
pub fn decode_remaining_length(buf: &[u8]) -> Result<(u32, usize), ProtocolError> {
    ...
    loop {
        if i >= 4 {
            // Â§2.2.3: Remaining Length is at most 4 bytes. Checked
        ...
        if i >= buf.len() {
            ...
            // continuation bit set â€” not necessarily malformed, just
        ...
        // stay well within u32 range â€” no overflow possible here.
...
// --- UTF-8 string field helpers, Â§1.5.3 ---
...
fn decode_utf8_string(buf: &[u8]) -> Result<(String, usize), ProtocolError> {
    if buf.len() < 2 {
        return Err(ProtocolError::MalformedPayload(
            "truncated UTF-8 string length prefix (Â§1.5.3)",
        ));
    }
    ...
    if buf.len() < end {
        return Err(ProtocolError::MalformedPayload(
            "UTF-8 string length prefix exceeds available bytes (Â§1.5.3)",
        ));
    }
    let s = std::str::from_utf8(&buf[start..end])
        .map_err(|_| ProtocolError::MalformedPayload("invalid UTF-8 in string field (Â§1.5.3)"))?
        .to_string();
    Ok((s, end))
}
...
// --- CONNECT / CONNACK, Â§3.1 / Â§3.2 ---

fn decode_connect(body: &[u8]) -> Result<ConnectPacket, ProtocolError> {
    let (protocol_name, mut pos) = decode_utf8_string(body)?;
    if protocol_name != "MQTT" {
        return Err(ProtocolError::MalformedPayload(
            "CONNECT protocol name must be \"MQTT\" (Â§3.1.2.1)",
        ));
    }

    if body.len() < pos + 1 {
        return Err(ProtocolError::MalformedPayload("truncated CONNECT protocol level (Â§3.1.2.2)"));
    }
    let protocol_level = body[pos];
    if protocol_level != 4 {
        return Err(ProtocolError::UnsupportedFeature(
            "only MQTT protocol level 4 (3.1.1) is supported (Â§3.1.2.2)",
        ));
    }
    pos += 1;

    if body.len() < pos + 1 {
        return Err(ProtocolError::MalformedPayload("truncated CONNECT connect flags (Â§3.1.2.3)"));
    }
    ...
    if will_flag || username_flag || password_flag {
        ...
        // ConnectPacket doc comment) â€” reject rather than silently
        // dropping them, since silently dropping would misrepresent
        // what the client asked for.
        return Err(ProtocolError::UnsupportedFeature(
            "CONNECT with will/username/password flags is out of core scope (PLAN.md Â§3)",
        ));
    }

    if body.len() < pos + 2 {
        return Err(ProtocolError::MalformedPayload("truncated CONNECT keep alive (Â§3.1.2.10)"));
    }
    let keep_alive_secs = u16::from_be_bytes([body[pos], body[pos + 1]]);
    pos += 2;

    let (client_id, pos_after_id) = decode_utf8_string(&body[pos..])
        .map_err(|_| ProtocolError::MalformedPayload("truncated CONNECT client identifier (Â§3.1.3.1)"))?;
    pos += pos_after_id;
...
/// `decode_utf8_string`) or emptiness beyond Â§4.7.3's "at least one
/// character" rule.
pub fn validate_topic_filter(filter: &str) -> Result<(), ProtocolError> {
    if filter.is_empty() {
        return Err(ProtocolError::MalformedPayload(
            "topic filter must be at least one character (Â§4.7.3)",
        ));
    }
    ...
    for (i, level) in levels.iter().enumerate() {
        if level.contains('#') {
            if *level != "#" {
                return Err(ProtocolError::MalformedPayload(
                    "'#' must occupy an entire topic level on its own (Â§4.7.1.2)",
                ));
            }
            if i != last_index {
                return Err(ProtocolError::MalformedPayload(
                    "'#' must be the last level in a topic filter (Â§4.7.1.2)",
                ));
            }
        } else if level.contains('+') && *level != "+" {
            return Err(ProtocolError::MalformedPayload(
                "'+' must occupy an entire topic level on its own (Â§4.7.1.3)",
            ));
        }
    }
    Ok(())
...
/// Does `topic` (a concrete published topic name â€” never itself
/// containing wildcards) match `filter` (a subscription's topic
/// filter, which may contain `+`/`#`)? Per Â§4.7.1's matching rules.
///
...
/// risk a stack overflow â€” see AI_GUARDRAILS.md rule 3.
///
...
pub fn topic_matches_filter(topic: &str, filter: &str) -> bool {
    ...
    while fi < filter_levels.len() {
        ...
        if ti >= topic_levels.len() {
            // Filter has more (non-'#') levels than the topic has â€”
            // can't match.
            return false;
        }
...
// --- SUBSCRIBE / SUBACK, Â§3.8 / Â§3.9 ---

fn decode_subscribe(body: &[u8]) -> Result<SubscribePacket, ProtocolError> {
    if body.len() < 2 {
        return Err(ProtocolError::MalformedPayload("truncated SUBSCRIBE packet identifier (Â§3.8.2)"));
    }
    ...
    if pos >= body.len() {
        return Err(ProtocolError::MalformedPayload(
            "SUBSCRIBE must contain at least one topic filter (Â§3.8.3)",
        ));
    }
    ...
    while pos < body.len() {
        let (topic, consumed) = decode_utf8_string(&body[pos..])
            .map_err(|_| ProtocolError::MalformedPayload("truncated SUBSCRIBE topic filter (Â§3.8.3)"))?;
        pos += consumed;
        validate_topic_filter(&topic)?;
        if pos >= body.len() {
            return Err(ProtocolError::MalformedPayload(
                "SUBSCRIBE topic filter missing requested QoS byte (Â§3.8.3)",
            ));
        }
        let qos = body[pos];
        if qos > 2 {
            return Err(ProtocolError::MalformedPayload(
                "SUBSCRIBE requested QoS must be 0, 1, or 2 (Â§3.8.3)",
            ));
        }
        pos += 1;
...
// --- UNSUBSCRIBE / UNSUBACK, Â§3.10 / Â§3.11 ---

fn decode_unsubscribe(body: &[u8]) -> Result<UnsubscribePacket, ProtocolError> {
    if body.len() < 2 {
        return Err(ProtocolError::MalformedPayload("truncated UNSUBSCRIBE packet identifier (Â§3.10.2)"));
    }
    let packet_id = u16::from_be_bytes([body[0], body[1]]);
    let mut pos = 2;
    ...
    if pos >= body.len() {
        return Err(ProtocolError::MalformedPayload(
            "UNSUBSCRIBE must contain at least one topic filter (Â§3.10.3)",
        ));
    }
    ...
    while pos < body.len() {
        let (topic, consumed) = decode_utf8_string(&body[pos..])
            .map_err(|_| ProtocolError::MalformedPayload("truncated UNSUBSCRIBE topic filter (Â§3.10.3)"))?;
        pos += consumed;
        validate_topic_filter(&topic)?;
...
// --- PUBLISH, Â§3.3 (QoS 0 core scope, QoS 1 extra scope PLAN.md Â§4 item 2) ---
...
fn decode_publish(flags: u8, body: &[u8]) -> Result<PublishPacket, ProtocolError> {
    ...
    if qos == 3 {
        return Err(ProtocolError::MalformedPayload(
            "PUBLISH QoS bits 11 is not a valid QoS value (Â§3.3.1.2)",
        ));
    }
    if qos == 2 {
        // QoS 2 is explicitly out of scope entirely (PLAN.md Â§4 item 2
        // only names QoS 1 as the extra).
        return Err(ProtocolError::UnsupportedFeature(
            "PUBLISH QoS 2 not supported (PLAN.md Â§4 scopes only QoS 0/1)",
        ));
    }

    let (topic, mut pos) = decode_utf8_string(body)
        .map_err(|_| ProtocolError::MalformedPayload("truncated PUBLISH topic name (Â§3.3.2.1)"))?;
    if topic.is_empty() {
        return Err(ProtocolError::MalformedPayload("PUBLISH topic name must not be empty (Â§4.7.3)"));
    }
    if topic.contains('+') || topic.contains('#') {
        // Wildcards are legal in a *subscription filter*, never in a
        // topic *name* being published to (Â§4.7.1).
        return Err(ProtocolError::MalformedPayload(
            "PUBLISH topic name must not contain wildcard characters (Â§4.7.1)",
        ));
    }

    // Packet Identifier is present only for QoS > 0 (Â§3.3.2.2), and per
    // Â§2.3.1 must be non-zero when present.
    let packet_id = if qos == 0 {
        None
    } else {
        if body.len() < pos + 2 {
            return Err(ProtocolError::MalformedPayload(
                "truncated PUBLISH packet identifier (Â§3.3.2.2)",
            ));
        }
        ...
        if id == 0 {
            return Err(ProtocolError::MalformedPayload(
                "PUBLISH packet identifier must be non-zero for QoS > 0 (Â§2.3.1)",
            ));
        }
        Some(id)
    };
...
// --- PUBACK, Â§3.4 (extra scope, PLAN.md Â§4 item 2) ---

fn decode_puback(body: &[u8]) -> Result<PubAckPacket, ProtocolError> {
    if body.len() != 2 {
        return Err(ProtocolError::MalformedPayload(
            "PUBACK variable header must be exactly a 2-byte packet identifier (Â§3.4.2)",
        ));
    }
    let packet_id = u16::from_be_bytes([body[0], body[1]]);
    if packet_id == 0 {
        return Err(ProtocolError::MalformedPayload(
            "PUBACK packet identifier must be non-zero (Â§2.3.1)",
        ));
    }
...
mod tests {
    ...
    // spec before Role B's implementation lands â€” see PLAN.md Â§8's
    // hour-0/parallel guidance. `#[should_panic]` is NOT the right tool
    // for testing rejection paths â€” decode() must return Err, not panic;
    ...
    fn remaining_length_rejects_five_continuation_bytes() {
        // Â§2.2.3: malformed if it would need a 5th byte.
    ...
    fn connect_truncated_mid_variable_header_is_malformed_not_panic() {
        // Only the protocol name, nothing else â€” decode() must return
    ...
    // --- SUBSCRIBE / UNSUBSCRIBE, Â§3.8 / Â§3.10 ---
    ...
    // --- PUBLISH, Â§3.3 (QoS 0 core scope) ---
    ...
    fn publish_empty_payload_is_valid() {
        // Â§3.3.3: a zero-length payload is legal.
        let original = MqttPacket::Publish(PublishPacket {
            topic: "t".to_string(),
            payload: vec![],
            qos: 0,
    ...
    fn publish_qos1_rejects_zero_packet_identifier() {
        let mut body = Vec::new();
        encode_utf8_string("t", &mut body);
        body.extend_from_slice(&0u16.to_be_bytes()); // packet id 0 â€” illegal
    ...
    fn puback_rejects_nonzero_flags() {
        ...
        // (Â§3.4.1) â€” construct one with a nonzero flag nibble by hand.
    ...
    fn matches_single_level_wildcard() {
        ...
        // '+' matches exactly one level â€” must not also swallow a
        // deeper level.
        assert!(!topic_matches_filter(
    ...
    fn matches_leading_slash_topics() {
        ...
        // spec's own worked example in Â§4.7.1.3).
    ...
    fn decode_only_consumes_one_packet_leaves_rest_buffered() {
        ...
        // decode() again on the remainder â€” verify that remainder is
    ...
    fn decode_max_remaining_length_never_panics() {
        ...
        // guard in decode() (Â§2.2.3 / AI_GUARDRAILS.md rule 3).
        //
        // The MQTT 3.1.1 spec max remaining length is 268,435,455, encoded
        // as four bytes [0xFF, 0xFF, 0xFF, 0x7F] (Â§2.2.3 Table 2.4).  We
        ...
        // that the code path runs to completion and returns an Err â€”
        // never panics â€” regardless of which Err variant is produced.
        ...
        // Must return an Err â€” never panic, never index out of bounds.
    ...
    fn decode_large_publish_payload_roundtrip() {
        ...
        // which encodes as 4 MQTT RL bytes (Â§2.2.3), well within the
...
// ===== BEGIN src/queue.rs =====
mod queue {
//! A small, bounded, drop-oldest message queue â€” the std-only substitute
...
//! wrong policy here â€” the broker thread must never stall because one
...
//! PLAN.md Â§3 (backpressure: bounded per-client outbound queue,
//! drop-oldest).
...
///
/// The trait is sealed to this module â€” nothing outside connection.rs
...
impl BrokerSend for Sender<BrokerMessage> {
    fn broker_send(&self, msg: BrokerMessage) {
        ...
        // connection is about to be torn down anyway â€” the caller's
...
fn dispatch_packet(
    ...
    match packet {
        MqttPacket::Connect(connect) => {
            ...
            // scope â€” see PLAN.md), this is where a rejection would be
        ...
        MqttPacket::Subscribe(sub) => {
            // MQTT 3.1.1 Â§3.9.3: SUBACK payload contains one return code
            // per topic filter in the SUBSCRIBE, in the same order.
            // Core scope: QoS 0 only, broker always accepts â†’ 0x00.
            ...
            for (topic, _qos) in sub.subscriptions {
                ...
                // Â§3.9.3 Table 3.4: 0x00 = Success â€“ Maximum QoS 0.
        ...
        MqttPacket::Unsubscribe(unsub) => {
            ...
            // MQTT 3.1.1 Â§3.11: the broker MUST send UNSUBACK in response
        ...
        MqttPacket::Publish(publish) => {
            // MQTT 3.1.1 Â§3.3.4: when a client publishes at QoS 1, the
            // broker MUST respond with PUBACK acknowledging that
            // specific packet identifier â€” this was missing (Role A
            ...
            // â€” it is not conditioned on the broker having found any
            // subscribers or having completed fan-out; PUBACK
            // acknowledges receipt by the broker, not delivery to
            // anyone downstream (Â§3.3.4 makes no such promise either).
            if publish.qos == 1 {
                ...
                // shouldn't (protocol.rs validates this) â€” silently
        ...
        MqttPacket::PubAck(ack) => {
            // ROLE B ADDED THIS ARM â€” flagging for Role A review, not
            // claiming ownership of connection.rs. Needed because
            // adding MqttPacket::PubAck (PLAN.md Â§4 item 2 / QoS 1)
            ...
            // broker.rs, so there's nothing to clear yet â€” this is
        ...
        MqttPacket::PingReq => {
            // Ping/pong is connection-local â€” no need to involve the
...
mod tests {
    ...
    fn pop_packet(q: &queue::QueueHandle<OutboundEvent>) -> MqttPacket {
        match q.pop_blocking() {
            ...
            None => panic!("outbound queue was empty â€” expected a packet"),
        }
    }

    // -----------------------------------------------------------------------
    // SUBSCRIBE â†’ SUBACK   (MQTT 3.1.1 Â§3.8 / Â§3.9)
    // -----------------------------------------------------------------------
    ...
    /// slice contains exactly one 0x00 (QoS 0 granted, Â§3.9.3 Table 3.4).
    #[test]
    fn subscribe_single_topic_produces_suback_with_correct_packet_id_and_return_code() {
        ...
        match pop_packet(&outbound) {
            MqttPacket::SubAck(ack) => {
                assert_eq!(
                    ack.packet_id, 5,
                    "SUBACK packet_id must match SUBSCRIBE packet_id (Â§3.9.2)"
                );
                assert_eq!(
                    ack.return_codes,
                    vec![0x00],
                    "one 0x00 return code per topic (Â§3.9.3 Table 3.4: QoS 0 granted)"
                );
            }
    ...
    /// (MQTT 3.1.1 Â§3.9.3: "The return codes must be listed in the same
        ...
        match pop_packet(&outbound) {
            MqttPacket::SubAck(ack) => {
                ...
                assert_eq!(
                    ack.return_codes,
                    vec![0x00, 0x00, 0x00],
                    "one return code per subscription, same order (Â§3.9.3)"
                );
            }
            other => panic!("expected SubAck, got {:?}", other),
        }
    ...
    /// (0xFFFF) is echoed correctly â€” no truncation or overflow.
    #[test]
    ...
    // -----------------------------------------------------------------------
    // UNSUBSCRIBE â†’ UNSUBACK   (MQTT 3.1.1 Â§3.10 / Â§3.11)
    // -----------------------------------------------------------------------

    /// A UNSUBSCRIBE must produce an UNSUBACK whose packet_id mirrors
    /// the UNSUBSCRIBE's packet_id (Â§3.11: fixed-format, no payload).
    #[test]
    fn unsubscribe_produces_unsuback_with_correct_packet_id() {
        ...
        match pop_packet(&outbound) {
            MqttPacket::UnsubAck(ack) => {
                assert_eq!(
                    ack.packet_id, 12,
                    "UNSUBACK packet_id must match UNSUBSCRIBE packet_id (Â§3.11.2)"
                );
            }
    ...
    /// to the broker, and exactly one UNSUBACK is sent (Â§3.11).
    #[test]
    ...
    // -----------------------------------------------------------------------
    // PUBLISH (QoS 1) â†’ PUBACK   (MQTT 3.1.1 Â§3.3.4)
    //
    ...
    // this â€” they only prove PUBACK's *wire format* is correct, not
    ...
    /// (Â§3.3.4) â€” this is the exact scenario that was broken.
    #[test]
    fn publish_qos1_produces_puback_with_matching_packet_id() {
        ...
        // Broker must still receive the Publish message for fan-out â€”
        ...
        match pop_packet(&outbound) {
            MqttPacket::PubAck(ack) => {
                assert_eq!(
                    ack.packet_id, 42,
                    "PUBACK packet_id must match the PUBLISH's packet_id (Â§3.3.4)"
                );
            }
            other => panic!("expected PubAck on the publishing client's own queue, got {:?}", other),
        }
        // Exactly one packet queued â€” no duplicate acks.
        assert!(
            outbound.pop_blocking().is_none(),
    ...
    /// A QoS 0 PUBLISH must NOT produce any PUBACK (Â§3.3.4 â€” acks only
    ...
    fn publish_qos0_produces_no_puback() {
        ...
        assert!(
            outbound.pop_blocking().is_none(),
            "QoS 0 PUBLISH must not produce any outbound packet (no PUBACK for QoS 0, Â§3.3.4)"
        );
    }

    /// Two QoS 1 PUBLISHes with different packet identifiers on the
    /// same connection must each get their own correctly-matched
    /// PUBACK, in order â€” guards against a fix that hardcodes or
...
//! `std::sync::mpsc` channel â€” see DECISIONS.md #1 for why (no data
//! races on the registry, per-topic publish order preserved, both by
//! construction, not by careful locking).
//!
//! Sharding (PLAN.md Â§4 item 3 / DECISIONS.md #1 upgrade path):
...
//! cheap at this scale â€” see DECISIONS.md #1 for the full reasoning.
//!
//! Wildcard subscriptions (PLAN.md Â§4 item 1 / DECISIONS.md #5):
...
//! by hash, unchanged â€” the fast path is preserved.
//!
//! Retained messages (PLAN.md Â§4 item 4):
...
//! (on empty payload, Â§3.3.1.3) the retained message for that topic â€” on the
...
/// Outbound queue capacity per connected client. Tunable â€” see PLAN.md Â§3
...
/// thread owning a disjoint topic subset â€” see PLAN.md Â§4 item 3.
...
/// the same shard â€” no cross-shard coordination required.
///
...
pub fn shard_for_topic(topic: &str, num_shards: usize) -> usize {
    ...
    // panicking on the % â€” shouldn't happen in practice since
...
/// that correctly (evicting the old session) is out of core scope â€” see
/// PLAN.md Â§4. An internal counter sidesteps that problem entirely for
...
/// is the stable contract connection.rs is built against â€” treat as
...
struct ClientState {
    /// Kept for logging only â€” the registry itself is keyed by
...
/// (see main.rs) â€” it owns the registry exclusively for as long as it
...
pub fn run_broker(rx: Receiver<BrokerMessage>) {
    ...
    // Retained messages (PLAN.md Â§4 item 4 / Â§3.3.1.3):
    // Maps an exact topic string â†’ the most recent retained PublishPacket
    // for that topic. Only concrete topic names appear as keys (PUBLISH
    // topic names never contain wildcards per Â§4.7.1). Access/mutation is
    ...
    for msg in rx {
        match msg {
            ...
            BrokerMessage::Subscribe { id, topic } => {
                ...
                // â”€â”€ Retained-message replay (PLAN.md Â§4 item 4, Â§3.3.1.3) â”€â”€
                // After registering, immediately deliver any retained message
                // whose topic matches this filter â€” exact comparison for plain
                // filters, topic_matches_filter for wildcards.
                //
                // The delivered packet has retain=true so the client can
                // distinguish a retained replay from a live message (Â§3.3.1.3).
            ...
            BrokerMessage::Publish { from: _, packet } => {
                // â”€â”€ Retained-message store update (Â§3.3.1.3) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                ...
                // burst â€” in practice the ordering is serial so either order
                // is correct, but storing first is more spec-natural.
                if packet.retain {
                    if packet.payload.is_empty() {
                        // Â§3.3.1.3: a retain=true PUBLISH with empty payload
                        // is a "delete" instruction â€” remove any stored
                ...
                // â”€â”€ Exact-match fast path (unchanged) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                ...
                // â”€â”€ Wildcard pass (PLAN.md Â§4 item 1 / DECISIONS.md #5) â”€â”€â”€â”€
                ...
                // cannot stack-overflow on adversarial inputs â€” GUARDRAILS Â§3.
...
// ---------------------------------------------------------------------------
// Sharded broker â€” PLAN.md Â§4 item 3
// ---------------------------------------------------------------------------
...
/// `Clone` â€” derive would work too but manual is explicit about cost (O(1)).
#[derive(Clone)]
...
impl ShardedBroker {
    ...
    /// - Subscribe / Unsubscribe / Publish â†’ single shard determined by
    ///   `shard_for_topic(topic, num_shards)` (topic field of the message).
    /// - Register / Disconnect â†’ **all** shards, because each shard needs
    ...
    ///   client, even those with zero subscriptions on that shard â€” this
        ...
        let routing = match &msg {
            ...
            | BrokerMessage::Unsubscribe { topic, .. } => {
                if is_wildcard_filter(topic) {
                    ...
                    // each shard can check them against incoming publishes â€”
        ...
        match routing {
            ...
            Routing::Broadcast => {
                ...
                match msg {
                    ...
                    // Publish is always SingleShard â€” unreachable here.
...
///
/// # Panics
/// Panics if `num_shards == 0` â€” at least one shard is required.
...
mod tests {
    ...
    fn disconnect_removes_subscriptions_across_multiple_topics() {
        ...
        // them after disconnecting â€” the single-topic case in
        ...
        // Publish to every topic the client was subscribed to â€” none
    ...
    ///
    /// **Scenario A â€” no-drop load:** 20 clients across 5 topics, 50 messages
    ...
    ///
    /// **Scenario B â€” over-capacity load (drop path):** 1 client subscribed to 1
    ...
    /// items â€” no more (no duplication), no fewer (no silent extra loss beyond
    ...
    fn stress_no_data_loss_beyond_drop_oldest() {
        // â”€â”€ Scenario A: no-drop load â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        // N clients, M topics. Each client subscribes to exactly one topic
        // (client i â†’ topic i % M). The broker must fan out exactly
        ...
        // Client i â†’ topic "stress/t{i % N_TOPICS}".
            ...
            for seq in 0..MSGS_PER_TOPIC {
                tx.send(BrokerMessage::Publish {
                    ...
                    packet: PublishPacket {
                        ...
                        // potential debugging â€” not verified here, just
                        // counted.
        ...
        // â”€â”€ Scenario B: over-capacity load (drop path) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        // after the broker finishes â€” the excess 20 are dropped (oldest-first),
    ...
    // -----------------------------------------------------------------------
    // Sharded-broker tests   (PLAN.md Â§4 item 3)
    // -----------------------------------------------------------------------

    /// With num_shards == 1, ShardedBroker must behave identically to a
    /// single run_broker call â€” this confirms the sharding layer introduces
    ...
    fn shard_isolation_publish_only_reaches_subscribed_shard() {
        ...
        // Publish to topic_shard0 via shard 0 â€” client MUST receive it.
        ...
        // Publish to topic_shard1 via shard 1 â€” client is NOT registered
    ...
    /// publishes correctly from both â€” the ShardedBroker's broadcast of
    ...
    // -----------------------------------------------------------------------
    // Wildcard fan-out tests   (PLAN.md Â§4 item 1 / DECISIONS.md #5)
    // -----------------------------------------------------------------------
    ...
    fn wildcard_cross_shard_unsubscribe_stops_delivery() {
        ...
        // Unsubscribe â€” must also broadcast to all shards.
    ...
    // -----------------------------------------------------------------------
    // Retained-message tests  (PLAN.md Â§4 item 4 / MQTT Â§3.3.1.3)
    // -----------------------------------------------------------------------
    ...
    /// must immediately receive the retained message (Â§3.3.1.3).
    #[test]
    fn retain_then_subscribe_delivers_retained_message() {
        ...
        match out.pop_blocking().expect("subscriber must receive retained message") {
            OutboundEvent::Packet(MqttPacket::Publish(p)) => {
                ...
                assert_eq!(p.payload, b"21C");
                assert!(p.retain, "delivered retained message must have retain=true (Â§3.3.1.3)");
            }
            _ => panic!("expected Publish"),
        }
        // No more messages.
    ...
    /// for that topic (Â§3.3.1.3). A subscriber arriving after the clear must
    /// receive nothing.
    #[test]
    ...
    /// â†’ routes to the same shard as the retained message). We then send the
    ...
    /// fully processed the BrokerMessage::Publish â€” meaning the retain store has
    /// been updated â€” before we send the wildcard Subscribe. This is race-free.
    #[test]
    fn retain_cross_shard_wildcard_subscribe_receives_retained_message() {
        ...
        // to a DIFFERENT shard â€” this forces the cross-shard scenario.
        let concrete_topic: String = (0u64..)
            .map(|i| format!("sensors/{}/temp", i))
            .find(|t| shard_for_topic(t, N) != wildcard_shard)
        ...
        // â”€â”€ Step 1: Register a "fence" subscriber on the concrete topic. â”€â”€â”€â”€
        ...
        // â”€â”€ Step 2: Send the retained Publish. â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        broker.send(BrokerMessage::Publish {
            from: 0,
            packet: make_publish(&concrete_topic, b"99C", true),
        }).unwrap();

        // â”€â”€ Step 3: Block until the fence receives the live publish. â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        // â”€â”€ Step 4: Now subscribe with the wildcard â€” retain is guaranteed. â”€
        let out = crate::queue::new::<OutboundEvent>(8);
        broker.send(BrokerMessage::Register {
            id: 50,
...
// ===== BEGIN src/main.rs (crate root; module declarations inlined above) =====
...
/// Hand-rolled CLI parsing â€” see STDLIB.md (`clap` substitution). Accepts
...
fn main() {
    ...
    // disjoint subset of topics â€” see DECISIONS.md #1 and PLAN.md Â§4 item 3.
