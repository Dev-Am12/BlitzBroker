...
Every package we'd normally install, and the std feature we used instead. **Owner: Role D compiles/enforces this, but whoever makes a substitution adds the entry the same day.** Entries below were cross-checked against `protocol.rs`, `broker.rs`, `connection.rs`, `queue.rs`, `logging.rs`, and `src/bin/blitzclient.rs` on 2026-08-31.
...
- `clap` → hand-rolled `std::env::args()` parsing — the CLI accepts `--host`, `--port`, and `--shards` (`main.rs`).
...
- MQTT retained-message store crates → shard-owned `std::collections::HashMap<String, PublishPacket>` alongside the subscriber registry — the retained store lives inside the same actor-owned shard map as subscriptions; no extra synchronization is needed and the same `mpsc` channel carries `Retain` variants (`broker.rs`).

- Criterion / benchmark harness crates → `std::time::Instant` and `std::sync::mpsc` inside `src/bin/shard_benchmark.rs` — throughput timing is measured with the monotonic clock from `std::time`; no external benchmarking framework is required for the shard-performance measurement (`shard_benchmark.rs`).